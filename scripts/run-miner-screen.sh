...
if [ -z "$WALLET_NAME" ] || [ -z "$WALLET_HOTKEY" ]; then
  ...
  echo "  export WALLET_NAME=your_coldkey WALLET_HOTKEY=default"
...
screen -dmS sn71 bash -c "cd '$PWD' && ./run-miner.sh 2>&1 | tee -a $LOG_FILE; echo 'Miner exited. Press Enter.'; read"
